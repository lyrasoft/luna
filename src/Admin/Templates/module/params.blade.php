...
<ul class="nav nav-tabs" role="tablist">
    @foreach ($fieldsets as $i => $fieldset)
        <li role="presentation" class="nav-item {{ $i !== 0 ? null : 'active' }}">
            <a class="nav-link {{ $i !== 0 ? null : 'active' }}" href="#fieldset-{{ $fieldset }}" aria-controls="home" role="tab" data-toggle="tab">
...
<div class="tab-content">
    @foreach ($fieldsets as $i => $fieldset)
        <div role="tabpanel" class="tab-pane fade {{ $i !== 0 ? null : 'in active show' }}" id="fieldset-{{ $fieldset }}">
