	...
	protected function doDefine(Form $form)
	{
		...
		$this->group('search', function (Form $form) use ($langPrefix)
		{
			...
			$this->list('field')
				...
				->option(Translator::translate($langPrefix . 'contact.field.title'), 'contact.subject')
				->option(Translator::translate($langPrefix . 'contact.field.title'), 'contact.name')
				->option(Translator::translate($langPrefix . 'contact.field.title'), 'user.name')
				->option(Translator::translate($langPrefix . 'contact.field.alias'), 'contact.content');
		...
		$this->group('filter', function(Form $form) use ($langPrefix)
		{
			...
			$this->list('contact.state')
				...
				->option('', '')
				->option(Translator::translate($langPrefix . 'contact.filter.state.select'), '')
