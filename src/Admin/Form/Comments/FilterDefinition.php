...
/**
 * The CommentsFilterDefinition class.
 *
 * @since  1.0
 */
	...
	public function define(Form $form)
	{
		...
		$form->wrap(null, 'search', function (Form $form)
		{
			...
			$form->add('field', new ListField)
				...
				->addOption(new Option(Translator::translate('admin.comment.field.title'), 'comment.title'))
				->addOption(new Option(Translator::translate('admin.comment.field.alias'), 'comment.alias'));
		 ...
		 *
		 * You can override filter actions in CommentsModel::configureFilters()
		 */
		$form->wrap(null, 'filter', function(Form $form)
		{
			// State
			$form->add('comment.state', new ListField)
				->label('State')
				// Add empty option to support single deselect button
				->addOption(new Option('', ''))
				->addOption(new Option(Translator::translate('admin.comment.filter.state.select'), ''))
